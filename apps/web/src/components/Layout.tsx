...
function ScrollToTop() {
  ...
  useEffect(() => {
    // Body must stay a block: a concise arrow would return scrollTo's value,
    // which React would then treat as a cleanup function.
    // 'instant' overrides the global smooth scroll so route changes jump
    // rather than visibly animating, while in-page anchors still glide.
      ...
      <div className="mx-auto grid max-w-7xl gap-10 px-6 py-16 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div>
          ...
          <ul className="mt-4 space-y-2.5 text-sm text-ink-muted">
            <li>
              <Link to="/gallery" className="transition-colors hover:text-ink">
                Gallery
              </Link>
            </li>
            <li>
