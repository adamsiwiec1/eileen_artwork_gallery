...
/**
 * Eileen's Flux.2-klein LoRA on a RunPod serverless endpoint.
 * Scale-to-zero: billed per GPU-second. Refinements re-render with the session seed.
 */
class RunpodImageProvider implements ImageProvider {
  readonly name = 'runpod';
  readonly supportsEditing = false;

  private endpoint = process.env.RUNPOD_ENDPOINT_ID?.trim() ?? '';
  private token = process.env.RUNPOD_API_KEY?.trim() ?? '';
  private steps = Number(process.env.EILEEN_STEPS ?? 8);
  private guidance = Number(process.env.EILEEN_GUIDANCE ?? 4.0);

  constructor() {
    if (!this.endpoint || !this.token) {
      throw new Error('IMAGE_PROVIDER=runpod requires RUNPOD_API_KEY and RUNPOD_ENDPOINT_ID');
    }
  }

  async render({ prompt, seed }: ImageRequest): Promise<GeneratedImage> {
    const started = await fetch(`https://api.runpod.ai/v2/${this.endpoint}/run`, {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({
        input: {
          prompt,
          seed,
          steps: Number.isFinite(this.steps) ? this.steps : 8,
          guidance: Number.isFinite(this.guidance) ? this.guidance : 4.0,
        },
      }),
    });

    if (started.status === 401 || started.status === 403) {
      throw Object.assign(new Error('RunPod rejected the API key or endpoint id.'), { status: 502 });
    }
    if (!started.ok) {
      const detail = await started.text();
      throw Object.assign(new Error(`RunPod queue returned ${started.status}: ${detail.slice(0, 240)}`), {
        status: 502,
      });
    }

    const queued = (await started.json()) as { id?: string; status?: string; error?: string };
    if (!queued.id) {
      throw new Error(queued.error || 'RunPod did not return a job id');
    }

    const output = await this.poll(queued.id);
    const url = asImageRef(output);
    if (!url) throw new Error('RunPod returned no image');
    return { url: url.startsWith('data:') ? url : `data:image/png;base64,${url}`, edited: false };
  }

  private async poll(jobId: string): Promise<unknown> {
    const deadline = Date.now() + 180_000;
    while (Date.now() < deadline) {
      const res = await fetch(`https://api.runpod.ai/v2/${this.endpoint}/status/${jobId}`, {
        headers: { Authorization: `Bearer ${this.token}` },
      });
      if (!res.ok) {
        throw Object.assign(new Error(`RunPod status failed (${res.status})`), { status: 502 });
      }
      const body = (await res.json()) as {
        status?: string;
        output?: { image?: unknown; error?: string } | unknown;
        error?: string;
      };
      if (body.status === 'COMPLETED') {
        const output = body.output;
        if (output && typeof output === 'object' && 'error' in output && (output as { error?: string }).error) {
          throw new Error(String((output as { error: string }).error));
        }
    ...
    throw Object.assign(new Error('RunPod job timed out waiting for an image'), { status: 504 });
  }
}
...
export function createImageProvider(): ImageProvider {
  ...
  const runpodEndpoint = process.env.RUNPOD_ENDPOINT_ID?.trim();

  const chosen =
    explicit ??
    (runpodKey && runpodEndpoint ? 'runpod' : undefined) ??
  ...
  switch (chosen) {
    ...
    case 'runpod':
      console.log('[images] runpod — Flux.2-klein LoRA serverless');
