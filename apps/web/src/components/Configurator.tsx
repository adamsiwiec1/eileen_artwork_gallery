...
import type { Catalog, CheckoutResult, Quote } from '../lib/types';
...
export function Configurator({ catalog, sessionId, imageUrl, onOrdered }: Props) {
  ...
  // The server is the only thing that prices an order; the client just asks.
  useEffect(() => {
    if (!mediumId || !sizeId || !rushTierId) return;
    let cancelled = false;

    api
      .quote({ mediumId, sizeId, rushTierId, framed })
  ...
  }, [mediumId, sizeId, rushTierId, framed]);
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      onOrdered(
        await api.checkout({ sessionId, mediumId, sizeId, rushTierId, framed, name, email }),
      );
    } catch (err) {
        ...
        <fieldset>
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-4">
            {catalog.sizes.map((s) => (
              ...
              >
                {/* Proportional swatch so relative scale is legible at a glance. */}
                <span
            ...
            {catalog.rushTiers.map((t) => (
              ...
              >
                ...
                <span className="shrink-0 text-sm text-ink-muted">
                  {t.surchargePct === 0 && t.flatFeeCents === 0
                    ? 'Included'
                    : `+${Math.round(t.surchargePct * 100)}%`}
                </span>
              </button>
            ))}
          </div>
      ...
      <aside className="lg:sticky lg:top-28 lg:self-start">
        ...
        >
          ...
          <p className="mt-3 text-center text-[0.68rem] leading-relaxed text-ink-muted/60">
            Payment processor not yet connected — checkout is simulated.
          </p>
        </form>
      </aside>
    </motion.div>
