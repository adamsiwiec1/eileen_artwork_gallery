...
function EaselPlaceholder({ busy }: { busy: boolean }) {
  return (
    <div className="relative grid aspect-square w-full place-items-center overflow-hidden rounded-sm border border-white/[0.08] bg-canvas-2">
      {busy ? (
        <>
          {/* Sweep suggesting a brush pass while the model renders. */}
          <motion.div
...
export function Studio() {
  ...
  // A prompt handed over from the hero starts generating on arrival. The ref
  // guards against StrictMode's double-effect firing two generations.
  useEffect(() => {
  ...
  return (
    <>
      <Seo
        title="The Studio — Design Your Commission | Eileen"
        description="Describe your painting, refine it in conversation, then choose medium, size and lead time. No account needed to start."
        path="/studio"
      />
              ...
              >
                <div className="lg:sticky lg:top-28 lg:self-start">
                  ...
                  {studio && !busy && (
                    <button
                      onClick={() => setStage('configuring')}
                      className="mt-6 w-full rounded-full bg-gilt py-3.5 text-sm font-medium text-canvas transition-all duration-300 hover:bg-gilt-bright hover:shadow-[0_0_34px_-6px] hover:shadow-gilt/60"
                    >
                ...
                <div className="flex h-[34rem] flex-col rounded-sm border border-white/[0.08] bg-canvas-2/40">
                  ...
                  {error && (
                    <p className="border-t border-white/[0.06] px-6 py-3 text-sm text-rose">{error}</p>
                  )}
                  ...
                  >
                    ...
                    <button
                      ...
                      className="shrink-0 rounded-full border border-gilt/40 bg-gilt/10 px-5 py-2 text-sm text-gilt-bright transition-all hover:bg-gilt/20 disabled:cursor-not-allowed disabled:opacity-30"
                    >
